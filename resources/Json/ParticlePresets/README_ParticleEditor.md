...
### GlobalVariables連携（GlobalVariablesタブ）
- **登録**: エミッターをGlobalVariablesに登録
- **適用**: GlobalVariablesから値を反映
- **JSON保存**: 設定をJSONファイルに永続化
...
// GlobalVariablesの適用
emitterManager_->ApplyGlobalVariables("PlayerFireEffect");
```

## ファイル構成
```
resources/Json/ParticlePresets/
├── Presets/              # 個別エミッタープリセット
│   ├── Fire.json
│   ├── Smoke.json
│   └── Magic.json
└── SampleScene.json      # シーンプリセットサンプル
```

## Tips
- エフェクトの微調整は実行中にリアルタイムで確認可能
- GlobalVariables連携により、ゲーム実行中でも調整可能
